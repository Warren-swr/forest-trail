...
- **驾驶反馈：**
  - 粒子：干土和碎石扬尘，泥地打滑甩泥点，涉水溅水花，共 420 个 CPU 粒子，合成一个动态网格绘制。粒子材质接收天空环境光补光并关闭镜面反射，避免缺少直接光照时变成黑团。干土尘偏暖褐、碎石尘偏灰褐，不透明度较低；干土与碎石路面超过 10.8 km/h 才起尘，每轮最多 20 个/秒，存活 1.2–1.8 秒，轻微扬起后下沉，避免形成显眼的白色烟带。
...
| --- | --- | --- | --- | --- |
| 高 | 1.5 | 2048，3 级，150 m | SSAO 完整渲染分辨率、24 次采样；泛光开 | 1.0× |
| 中 | 1.0 | 1536，3 级，120 m | SSAO 完整渲染分辨率、16 次采样；泛光开 | 0.8× |
| 低 | 0.8 | 1024，2 级，90 m | 关（MSAA 也关） | 0.6× |

- **接触阴影：**SSAO 强度 0.45、半径参数 1.2、power 3，开启深度感知模糊。完整分辨率和更多采样减少车底、草根附近的细条纹；保留原先的遮蔽范围与强度，并以修复前的整体明暗为参考校准衰减，保持轮拱、底盘和植被的立体感。AO 采样保持固定，避免未启用 TAA 时的逐帧噪声闪烁；模型边缘继续使用 MSAA。
