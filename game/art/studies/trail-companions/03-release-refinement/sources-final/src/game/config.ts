...
const SCOUT: VehicleSpec = {
  ...
  hardpointY: 0.3,
  /** spring free length (hardpoint → wheel centre) */
  // Longer droop and mass-matched sag keep the short body settled over the tyres.
  restLength: 0.50,
  /** full bump: min length */
  minLength: 0.12,
  springK: 16000,
  ...
  colliders: [
    ...
    { half: [0.43, 0.43, 0.16], at: [0, 0.81, 2.135] }, // full-size spare on the swing carrier
    { half: [0.92, 0.2, 1.7], at: [0, 0.68, 0.05] }, // tub between belt line and sills
    { half: [0.86, 0.37, 1.18], at: [0, 1.19, 0.62], friction: 0.5 }, // retained release cabin
  ],
  axleHalf: 0.5,
...
const RANGER: VehicleSpec = {
  ...
  wheelMass: 25,
  hardpointY: 0.3,
  // The lighter pickup needs less spring force and restrained rear roll coupling.
  restLength: 0.48,
  minLength: 0.13,
  springK: 15500,
  damperBump: 2800,
  ...
  colliders: [
    ...
    { half: [0.86, 0.31, 0.20], at: [0, 0.48, -1.975] }, // retained chrome bumper
    { half: [0.84, 0.30, 0.33], at: [0, 0.475, 2.09] }, // step bumper and rear overhang
    { half: [0.80, 0.277, 0.595], at: [0, 1.065, -0.335], friction: 0.5 }, // short release cab
    { half: [0.82, 0.16, 0.96], at: [0, 0.61, 1.32] }, // original bed depth and equipment
  ],
  axleHalf: 0.48,
