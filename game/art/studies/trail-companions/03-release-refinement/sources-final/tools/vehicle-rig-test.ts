...
for (const id of VEHICLE_ORDER) {
  ...
  if (['trail-v2-refinement/1', 'trail-companions/1', 'published-release-refinement/1'].includes(info.style)) {
...
const output = process.env.RIG_OUT;
writeFileSync(output ?? 'tools/vehicle-rig-results.json', JSON.stringify({ passed: results.length, results }, null, 2) + '\n',
  output ? { flag: 'wx' } : undefined);
