...
def greenhouse(mb, s):
    ...
    def details(bm, kw, lo, hi, i):
        if i == 0 or (scout and i == 1):
            return False
        if not scout and hi.z - lo.z > .25 and .50 < lo.y < hi.y < .70 and abs((lo.x + hi.x) / 2) > .74:
            return False  # replace the release's short, floating quarter divider
        return True
    ...
    for side in (-1, 1):
        ...
        if not scout:
            def quarter(z, y, offset=0):
                x = .80 - .065 * (z - .80) / .48 - .007 + offset
                return (side * x, y, z)
            # The divider terminates in the lower seal and roof header, with
            # a rubber bedding strip behind the thin metal face.
...
def ranger_rollbar(mb):
    # Retain the two guarded lamps, diagonal stays and feet. Replace just the
    # release's five butted tube segments with one continuously bent hoop.
    retain(mb, RANGER.add_rollbar, lambda bm, kw, lo, hi, i: i >= 5)
    y, radius = -.48, .095
    pts = [(-.70, y, .77), (-.70, y, 1.285)]
    for k in range(1, 13):
        a = math.pi - math.pi / 2 * k / 12
        pts.append((-.605 + radius * math.cos(a), y, 1.285 + radius * math.sin(a)))
    pts.append((.605, y, 1.38))
    for k in range(1, 13):
        a = math.pi / 2 * (1 - k / 12)
        pts.append((.605 + radius * math.cos(a), y, 1.285 + radius * math.sin(a)))
    pts.append((.70, y, .77))
    F.path(mb, pts, .030, 'Trim', 16)
