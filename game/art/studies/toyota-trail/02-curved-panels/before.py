...
STUDY = os.path.join(C.ART, 'studies', 'toyota-trail')
COWL, HEADER = .74, .51
...
def add_hood(mb):
    # Closed, gently crowned bonnet. Its lower edge sits inside the tub;
    # a narrow perimeter groove provides separation without an open black gap.
    verts, faces = [], []
    nx, ny = 16, 6
    for j in range(ny + 1):
        t = j / ny
        y = COWL + .014 + (FRONT - COWL - .027) * t
        for i in range(nx + 1):
            u = 2 * i / nx - 1
            z = BELT + .009 + .027 * (1 - u ** 4) - .003 * t
            verts.append((u * .864, y, z))
    for j in range(ny):
        for i in range(nx):
            a = j * (nx + 1) + i
    ...
    start = len(verts)
    verts += [(verts[k][0], verts[k][1], BELT - .006) for k in edge]
    ...
    mb.add(bm, mat='Paint', shade='auto', angle=28)
    for side in (-1, 1):
        F.box(mb, (side * .868, (COWL + FRONT) / 2, BELT + .001), (.004, FRONT - COWL - .025, .004), 'PanelGap', .001, 1)
    F.box(mb, (0, COWL + .009, BELT + .010), (1.73, .007, .008), 'PanelGap', .002, 1)
    F.box(mb, (0, COWL - .055, BELT + .005), (1.74, .108, .032), 'Paint', .010, 3)
    for side in (-1, 1):
        for k in range(5):
            F.box(mb, (side * (.32 + k * .036), COWL - .056, BELT + .023), (.021, .045, .006), 'Trim', .002, 1)


def add_greenhouse(mb):
    # A single inset shell gives consistent jambs, continuous pillars and dark
    # window recesses. This is deliberately V2's construction, not V3's tubes.
    ...
    # Longitudinal roof ribs, modest crown and a continuous rain gutter.
    F.box(mb, (0, (HEADER - 2.055) / 2, zt + .010), (1.645, HEADER + 2.105, .057), 'PaintAccent', .022, 3)
    for x in (-.55, -.28, .28, .55):
        F.box(mb, (x, -.81, zt + .041), (.022, 2.40, .012), 'PaintAccent', .006, 2)
    for side in (-1, 1):
        F.box(mb, (side * .841, (HEADER - 2.065) / 2, zt - .010), (.024, HEADER + 2.155, .027), 'Paint', .006, 2)
...
def main():
    ...
    info = dict(id='toyota', fidelity=3, style='trail-v2-refinement/1', reference='Toyota Land Cruiser FJ60',
                rigFile='art/studies/toyota-trail/rig.json',
