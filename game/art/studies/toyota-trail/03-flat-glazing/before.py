...
def greenhouse_point(p):
    """Bend the entire window surround together, including seals and glazing.

    A-pillar is nearly upright at its foot and leans progressively into the roof.
    The side wall has gentle tumblehome, rather than a constant planar slope.
    """
    x, y, z = p
    t = max(0, min(1, (z - BELT) / (ROOF - BELT)))
    front = COWL + (HEADER - COWL) * t
    rear = -2.10 + .045 * t
    u = max(0, min(1, (y - rear) / (front - rear)))
    original_width = .885 + (.818 - .885) * t
    width = original_width + .024 * math.sin(math.pi * t) - .006 * math.sin(2 * math.pi * t)
    new_front = COWL * (1 - t) ** 3 + 3 * COWL * (1 - t) ** 2 * t + 3 * .625 * (1 - t) * t * t + HEADER * t ** 3
    y += (new_front - front) * u ** 4
    y += .012 * math.sin(math.pi * t) * (1 - u) ** 4
    y += .010 * max(0, 1 - (x / original_width) ** 2) * math.sin(math.pi * t) * u ** 6
    return (x * width / original_width, y, z)
...
def round_window(bm, face, radius=.026, segments=6):
    """Round a planar window outline while sharing edges with its jamb.

    A generic vertex bevel also rounds the edges running into the recess, which
    leaves small corner tabs. Split the perimeter explicitly and fill only the
    corner outside the glass with painted sheet instead.
    """
    ...
    for i, a in enumerate(corners):
        ...
        edge = next(e for e in edges if a in e.verts and b in e.verts)
        length = (b.co - a.co).length
        fraction = min(.18, radius / length)
        _, near_a = bmesh.utils.edge_split(edge, a, fraction)
        remaining = next(e for e in near_a.link_edges if b in e.verts)
        _, near_b = bmesh.utils.edge_split(remaining, b, fraction / (1 - fraction))
        outgoing[i], incoming[(i + 1) % len(corners)] = near_a, near_b
    loop = []
    for i, corner in enumerate(corners):
        ...
        for j in range(1, segments):
            t = j / segments
            arc.append(bm.verts.new(a.co * (1 - t) ** 2 + corner.co * (2 * (1 - t) * t) + b.co * t * t))
        arc.append(b)
        for j in range(segments):
            bm.faces.new([corner, arc[j + 1], arc[j]])
        loop.extend(arc)
    return bm.faces.new(loop)


def add_greenhouse(mb):
    # Inset a connected shell first, then curve all of its panels and jambs as one.
    ...
    bevel_faces = set(C.bevel(bm, .035, 5, edges).get('faces', []))
    bm.normal_update()
    ...
    # Radiused window corners round the pillar-to-header transitions. Doing this
    # before the final seal inset keeps a constant-width rubber lip around them.
    wins = [round_window(bm, f) for f in wins]
    assert len(wins) == 8, 'Expected eight continuous glass panes'
    bm.normal_update()
    seals = set(bmesh.ops.inset_individual(bm, faces=wins, thickness=.009, depth=-.003)['faces'])
    windows = set(wins)
    for f in bm.faces:
        f.material_index = 1 if f in windows else 2 if f in seals else 0
    # Give every pane interior vertices too, so its crown is represented by real
    # geometry, not merely by a curved perimeter and an arbitrary ngon diagonal.
    for f in bmesh.ops.poke(bm, faces=wins)['faces']:
        f.material_index = 1
    # Subdivide the long spans that actually need curvature; tiny corner fillets
    # already have sufficient segments and should not inflate the whole body.
    glass_edges = {e for f in bm.faces if f.material_index == 1 for e in f.edges}
    bmesh.ops.subdivide_edges(bm, edges=[e for e in bm.edges if e in glass_edges or e.calc_length() > .09], cuts=9, use_grid_fill=True)
    bm.normal_update()
    # Analytic surface tangents keep the glass reflection continuous across its
    # fan topology. Area-weighted triangle normals leave a visible diagonal on a
    # shallow curved pane even when its vertices lie on the correct surface.
    glass_normals = {}
    for i, face in enumerate(bm.faces):
        if face.material_index != 1:
            continue
        tangent = face.normal.cross(Vector((0, 0, 1))).normalized() * .0001
        bitangent = face.normal.cross(tangent).normalized() * .0001
        normals = []
        for v in face.verts:
            a = Vector(greenhouse_point(v.co + tangent)) - Vector(greenhouse_point(v.co - tangent))
            b = Vector(greenhouse_point(v.co + bitangent)) - Vector(greenhouse_point(v.co - bitangent))
            normals.append(a.cross(b).normalized())
        glass_normals[i] = normals
    for v in bm.verts:
        v.co = greenhouse_point(v.co)
    bmesh.ops.recalc_face_normals(bm, faces=bm.faces)
    first_face = len(mb.F)
    mb.add(bm, mat_fn=lambda f: ('Paint', 'Glass', 'Rubber')[f.material_index], shade='auto', angle=38)
    for i, normals in glass_normals.items():
        mb.LN[first_face + i] = normals
    ...
    # Attach the wipers and quarter-light divider to the same curved surface.
    for x in (-.38, .34):
        a = (x, COWL - .018, zb + .034)
        b = (x + .17, COWL - .058, zb + .139)
        F.path(mb, [greenhouse_point(tuple(a[j] + (b[j] - a[j]) * t for j in range(3))) for t in (i / 8 for i in range(9))], .006, 'Trim', 8)
        F.path(mb, [greenhouse_point((b[0] - .15 + i * .03, b[1], b[2])) for i in range(11)], .008, 'Rubber', 8)
    for side in (-1, 1):
        a, b = (side * .875, .46, zb + .052), (side * .822, .39, zt - .044)
        F.path(mb, [greenhouse_point(tuple(a[j] + (b[j] - a[j]) * t for j in range(3))) for t in (i / 12 for i in range(13))], .007, 'Paint', 8)
...
def add_front(mb):
    ...
    y, z = FRONT, .652
    F.box(mb, (0, y + .009, z), (1.75, .061, .407), 'Trim', .014, 3)
    for zz in (.470, .837):
        F.box(mb, (0, y + .044, zz), (1.72, .023, .016), 'Metal', .004, 2)
    for k in range(5):
        F.box(mb, (0, y + .047, .501 + k * .060), (1.02, .023, .017), 'Metal', .003, 2)
    for x in (-.34, 0, .34):
    ...
    for side in (-1, 1):
        ...
        F.box(mb, (side * .821, y + .045, .671), (.092, .035, .176), 'LampAmber', .008, 3)
        F.box(mb, (side * .821, y + .046, .555), (.092, .036, .049), 'Lamp', .006, 2)
        F.box(mb, (side * .896, 1.73, .708), (.016, .083, .032), 'LampAmber', .004, 2)
    ...
    F.box(mb, (0, y - .004, .865), (1.77, .036, .026), 'Paint', .007, 2)
...
def add_sides(mb):
    ...
    for side in (-1, 1):
        ...
        F.text(mb, 'LAND CRUISER', (side * .905, 1.135, .807), .025, 'right' if side > 0 else 'left', 'Metal')
...
def main():
    ...
    lamps = dict(L.LAMPS)
    lamps['head'] = [(-.624, FRONT + .073, .652), (.624, FRONT + .073, .652)]
