// Toyota-only study. Compare the saved first sample with the curved-panel sample in one
...
const revision = '02-curved-panels';
...
const report = { capturedAt: new Date().toISOString(), viewport: { width: 1600, height: 1000 },
  revision,
  method: 'Unmodified production WebGL canvas. The first Toyota sample and curved-panel sample share one frozen vehicle pose, environment time, camera and sand paint. Baseline comparison concerns appearance only; the subsequent candidate drive runs real physics.',
