...
def add_sides(mb):
    ...
    for side in (-1, 1):
        for yc, zr, zf in ((-WB / 2, .24, .30), (WB / 2, .30, .24)):
            outer = arch(yc, r=ARCH_R + .10, zr=zr, zf=zf)
            inner = arch(yc, r=ARCH_R, zr=zr, zf=zf)[::-1]
            bm = C.bm_extrude_poly(outer + inner, .16, axis='X', center=side * .94)
            outer_edges = [e for e in bm.edges if all(abs(v.co.x) > 1.015 for v in e.verts)]
