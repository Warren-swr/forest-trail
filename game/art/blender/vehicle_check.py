...
def mounts(dims):
    ...
    return {'Spring': (sp, 0.10), 'Driveshaft': (tc, 0.09)}
...
def run(parts, dims, shrink=0.985, verbose=True):
    ...
    for name, pose in poses(dims).items():
        ...
        movers = [o for o in objs if o is not body]
        bt = _tree(body)
        res = {}
        mt = mounts(dims)
        for o in movers:
            ...
            if kind in mt:
                pts, rad = mt[kind]
                pairs = [p for p in pairs if min((_centre(o, p[0]) - q).length for q in pts) > rad]
            n = len(pairs)
            if n:
        ...
        others = [o for o in movers if o.data in (parts['Spring'].data, parts['AxleFront'].data, parts['AxleRear'].data)]
        for w in wheels:
            wt = _tree(w, shrink)
            for o in others:
